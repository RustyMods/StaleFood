...
namespace StaleFood.Managers
{
    ...
    public static class MaterialReplacer
    {
        ...
        private static void ReplaceAllMaterialsWithOriginal()
        {
            ...
            foreach (Renderer renderer in ObjectToSwap.Keys.SelectMany(gameObject => gameObject.GetComponentsInChildren<Renderer>(true)))
            {
                if (!renderer.gameObject) continue;
                // if (!ObjectToSwap.TryGetValue(renderer.gameObject, out bool jotunnPrefabFlag)) continue;
                ...
                foreach (Material t in renderer.sharedMaterials)
                {
                    ...
                    if (!t.name.StartsWith(replacementString)) continue;
                    string matName = t.name.Replace(" (Instance)", string.Empty).Replace(replacementString, "");
